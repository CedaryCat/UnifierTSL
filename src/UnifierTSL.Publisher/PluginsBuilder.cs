...
public class PluginsBuilder(string RelativePluginProjectDir)
{
    ...
    static ImmutableArray<string> Build(string rid, string solutionDir, List<FileInfo> projects) {
        ...
        for (int i = 0; i < projects.Count; i++) {
            ...
            var outputDir = Path.Combine(Utils.Solution.DefaultOutputPath, "plugins-publish", projectName);
            ...
            var startInfo = new ProcessStartInfo {
                FileName = "dotnet",
                Arguments = $"build \"{project.FullName}\" -c {Utils.Solution.configuration} -o \"{outputDir}\"",
