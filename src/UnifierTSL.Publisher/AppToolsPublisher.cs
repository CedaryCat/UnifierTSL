...
public class AppToolsPublisher(string[] appProjectPaths)
{
    ...
    public ImmutableArray<string> PublishApps(string rid) {
        ...
        foreach(var relativePath in appProjectPaths) {
            ...
            var outputDir = Path.Combine(Utils.Solution.DefaultOutputPath, "apps-publish", Path.GetFileNameWithoutExtension(relativePath));
            ...
            var startInfo = new ProcessStartInfo {
                FileName = "dotnet",
                Arguments = $"publish \"{projectPath}\" -c {Utils.Solution.configuration} -p:PublishSingleFile=true -p:SelfContained=false -o \"{outputDir}\" -r {rid}",
