...
public class CoreAppBuilder(string relativeProjectPath)
{
    ...
    public CoreAppBuilderResult Build(string rid) {
        var solutionDir = Utils.Solution.SolutionRoot;
        ...
        var publishDir = Path.Combine(Utils.Solution.DefaultOutputPath, "core-publish", projectName);
        ...
        var buildProcess = Process.Start(new ProcessStartInfo {
            FileName = "dotnet",
            Arguments = $"build \"{projectPath}\" -c {Utils.Solution.configuration}",
        ...
        var buildDir = Path.Combine(projectDir, "bin", Utils.Solution.configuration, Utils.DotnetSdk.GetTFMString());
        ...
        var appHostTemplate = Utils.DotnetSdk.GetBestMatchedAppHostPath(rid);

        var executable = Path.Combine(publishDir, projectName + Utils.File.ExecutableExtension(rid));
