	...
	public class Rest : IDisposable
	{
		...
		public virtual void Start()
		{
			try
			{
				...
				{
					DegradeBucket();
				}, null, TimeSpan.Zero, TimeSpan.FromMinutes(Math.Max(TShock.Config.GlobalSettings.RESTRequestBucketDecreaseIntervalMinutes, 1)));

			}
			catch (Exception ex)
			{
				TShock.Log.LogUnexpectedException(
					category: "Fatal Startup Exception",
                    message: GetString("Invalid REST configuration: \nYou may already have a REST service bound to port {0}. \nPlease adjust your configuration and restart the server. \nPress any key to exit.", Port),
					ex: ex);
		...
		protected virtual object ExecuteCommand(RestCommand cmd, RestVerbs verbs, IParameterCollection parms, IRequest request, IHttpContext context)
		{
			...
			if (cmd.DoLog && TShock.Config.GlobalSettings.LogRest)
			{
				...
				TShock.Log.Info(GetString($"Anonymous requested REST endpoint: {endpoint}"));
			}
