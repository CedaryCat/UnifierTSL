...
namespace Rests
{
	public class SecureRest : Rest
	{
			...
			: base(ip, port)
		{
			...
			foreach (KeyValuePair<string, TokenData> t in TShock.Config.GlobalSettings.ApplicationRestTokens)
			{
				AppTokens.Add(t.Key, t.Value);
			}
		}
		...
		private RestObject NewTokenInternal(string username, string password, IHttpContext context)
		{
			...
			{
				if (tokens >= TShock.Config.GlobalSettings.RESTMaximumRequestsPerInterval)
				{
					TShock.Log.Error(GetString("A REST login from {0} was blocked as it currently has {1} rate-limit tokens and is at the RESTMaximumRequestsPerInterval threshold.", context.RemoteEndPoint.Address.ToString(), tokens));
		...
		protected override object ExecuteCommand(RestCommand cmd, RestVerbs verbs, IParameterCollection parms, IRequest request, IHttpContext context)
		{
			...
			if (cmd.DoLog && TShock.Config.GlobalSettings.LogRest)
