...
namespace TShockAPI
{
	...
	public static class Utils
	{
		...
		public static void Broadcast(ServerContext server, string msg, byte red, byte green, byte blue)
		{
			var serverPlr = server.GetExtension<TSServerPlayer>();
            serverPlr.BCMessage(msg, red, green, blue);
		}

		/// <summary>>Broadcast - Broadcasts a message to all players on the server, as well as the server console, and the logs.</summary>
		/// <param name="msg">msg - The message to send</param>
		/// <param name="color">color - The color object for supported destinations.</param>
		public static void Broadcast(ServerContext server, string msg, Color color)
		{
			Broadcast(server, msg, color.R, color.G, color.B);
		}
		...
		/// <param name="green">green - The amount of green (0-255) in the color for supported destinations.</param>
		/// <param name="blue">blue - The amount of blue (0-255) in the color for the supported destinations.</param>
		public static void Broadcast(ServerContext server, int ply, string msg, byte red, byte green, byte blue)
		{
			var serverPlr = server.GetExtension<TSServerPlayer>();
            serverPlr.SendMessageFromPlayer(msg, red, green, blue, ply);
            server.Log.Info(category: "Broadcast", message: GetString("{0}: {1}", server.Main.player[ply].name, msg));
		}
		...
		public static void SendLogs(ServerContext server, string log, Color color, TSPlayer? excludedPlayer = null)
		{
			server.Log.Info(log);
			//server.GetExtension<TSServerPlayer>().SendMessage(log, color);
			var setting = TShock.Config.GetServerSettings(server.Name);
			foreach (TSPlayer player in TShock.Players) {
                if (player is null) {
                    continue;
                }
                if (player.GetCurrentServer() != server) {
					continue;
				}
        ...
        /// </summary>
        /// <returns>The number of active players on the server.</returns>
        public static int GetActivePlayerCount(ServerContext server) {
            return TShock.Players.Count(p => null != p && p.Active && p.FinishedHandshake && p.GetCurrentServer() == server);
        }
		...
		public static void GetRandomClearTileWithInRange(ServerContext server, int startTileX, int startTileY, int tileXRange, int tileYRange,
																							out int tileX, out int tileY)
		{
			...
			do
			{
				...
				j++;
			} while (TilePlacementValid(server, tileX, tileY) && TileSolid(server, tileX, tileY));
		}
		...
		public static bool TilePlacementValid(ServerContext server, int tileX, int tileY)
		{
			return tileX >= 0 && tileX < server.Main.maxTilesX && tileY >= 0 && tileY < server.Main.maxTilesY;
		}
		...
		public static bool TileSolid(ServerContext server, int tileX, int tileY)
		{
			return TilePlacementValid(server, tileX, tileY) && server.Main.tile[tileX, tileY].IsNotNull &&
				server.Main.tile[tileX, tileY].active() && server.Main.tileSolid[server.Main.tile[tileX, tileY].type] &&
				!server.Main.tile[tileX, tileY].inActive() && !server.Main.tile[tileX, tileY].halfBrick() &&
				server.Main.tile[tileX, tileY].slope() == 0 && server.Main.tile[tileX, tileY].type != TileID.Bubble;
		}
		...
		{
			Item item = new Item();
			item.netDefaults(TShock.ServerSample, id);
			return item;
		}
		...
		public static Item GetItemFromTag(string tag)
		{
			...
			if (!match.Success)
				return null;
			Item item = new Item();
			item.netDefaults(TShock.ServerSample, Int32.Parse(match.Groups["NetID"].Value));
		...
		{
			NPC npc = new NPC();
			npc.SetDefaults(TShock.ServerSample, id);
			return npc;
		}
		...
		public static void StopServer(ServerContext server, bool save = true, string reason = "Server shutting down!")
		{
			if (save)
		...
		public static bool HasWorldReachedMaxChests(ServerContext server)
		{
			for (int i = 0; i < server.Main.chest.Length; i++)
			{
				if (server.Main.chest[i] == null)
					return false;
			}
			return true;
		}
		...
		/// <param name="seconds">The seconds.</param>
		/// <returns>Whether the string was parsed successfully.</returns>
		public static bool TryParseTime(string str, out ulong seconds)
		{
			seconds = 0;
		...
		public static int SearchProjectile(ServerContext server, short identity, int owner)
		{
			for (int i = 0; i < Main.maxProjectiles; i++)
			{
				if (server.Main.projectile[i].identity == identity && server.Main.projectile[i].owner == owner)
					return i;
			}
		...
		public static IEnumerable<Point> EnumerateRegionBoundaries(Rectangle regionArea)
		{
		...
		public static Color? DecodeColor(int? encodedColor)
		{
		...
		public static int? EncodeBoolArray(bool[] bools)
		{
			if (bools == null)
				return null;
		...
		public static bool[] DecodeBoolArray(int? encodedbools)
		{
		...
		public static byte? EncodeBitsByte(BitsByte? bitsByte)
		{
			if (bitsByte == null)
				return null;
		...
		public static BitsByte? DecodeBitsByte(int? encodedBitsByte)
		{
		...
		public static string ItemTag(Item item)
		{
			int netID = item.type;
		...
		/// </returns>
		public static List<Point> GetMassWireOperationRange(Point start, Point end, bool direction = false)
		{
		...
		public static void Dump(bool exit = true)
		{
			// Lang.setLang(true);
		...
		internal static void DumpPermissionMatrix(string path)
		{
		...
		internal static void StartInvasion(ServerContext server, int type)
		{
			int invasionSize = 0;
			var setting = TShock.Config.GetServerSettings(server.Name);

			if (setting.InfiniteInvasion)
			{
			...
			{
				invasionSize = 100 + (setting.InvasionMultiplier * GetActivePlayerCount());
			}

			// Order matters
			// StartInvasion will reset the invasion size

			server.Main.StartInvasion(type);
			...
			server.Main.invasionSizeStart = invasionSize;
			server.Main.invasionSize = invasionSize;
		}

		/// <summary>Verifies that each stack in each chest is valid and not over the max stack count.</summary>
		internal static void FixChestStacks(ServerContext server)
		{
			var setting = TShock.Config.GetServerSettings(server.Name);

			if (setting.IgnoreChestStacksOnLoad)
				return;

			foreach (Chest chest in server.Main.chest)
			{
				if (chest != null)
				{
		...
		public static bool IsInSpawn(ServerContext server, int x, int y)
		{
			Vector2 tile = new Vector2(x, y);
			Vector2 spawn = new Vector2(server.Main.spawnTileX, server.Main.spawnTileY);
			return Distance(spawn, tile) <= TShock.Config.GetServerSettings(server.Name).SpawnProtectionRadius;
		}

        internal static void ComputeMaxStyles() {
            var item = new Item();
            for (int i = 0; i < Terraria.ID.ItemID.Count; i++) {
                item.netDefaults(TShock.ServerSample, i);
                if (item.placeStyle >= 0) {
                    if (Bouncer.MaxPlaceStyles.ContainsKey(item.createTile)) {
                        if (item.placeStyle > Bouncer.MaxPlaceStyles[item.createTile])
                            Bouncer.MaxPlaceStyles[item.createTile] = item.placeStyle;
                    }
                    else
                        Bouncer.MaxPlaceStyles.Add(item.createTile, item.placeStyle);
                }
            }
        }
    }
