...
public class ReduceConsoleSpam : Module
{
	public override void Initialise() {
        On.OTAPI.HooksSystemContext.MainSystemContext.InvokeStatusTextChange += OnInvokeStatusTextChange;
	}

    private string OnInvokeStatusTextChange(
		On.OTAPI.HooksSystemContext.MainSystemContext.orig_InvokeStatusTextChange orig,
		OTAPI.HooksSystemContext.MainSystemContext self,
		string value) {

		var server = self.root;
        void WriteIfChange(string text) {
            if (_lastStatusText != text) {
                server.Console.WriteLine(text); // write it manually instead of terraria which causes double writes
                _lastStatusText = text;
            }
        }
        bool replace(string text) {
            if (value.StartsWith(text)) {
                var segment = value.Substring(0, text.Length);
        ...
        var master = value.IndexOf(FindMaster);
        if (master > -1) {
            var sub = value.LastIndexOf(FindSub);
            if (master > -1 && sub > master) {
                var mprogress = value.Substring(0, master + 1/*%*/);
                var sprogress = value.Substring(sub + FindSub.Length);
                if (mprogress.EndsWith("%") && sprogress.EndsWith("%")) {
                    var text = value.Substring(master + FindMaster.Length, sub - master - FindMaster.Length).Trim();
