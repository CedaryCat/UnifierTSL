    ...
    public readonly struct TraceId
    {
        ...
        public static unsafe TraceId Parse(ReadOnlySpan<char> hex) {
            if (hex.Length != Size * 2)
                throw new ArgumentException(GetString($"TraceId hex string must be {Size * 2} characters."), nameof(hex));
            ...
            if (status != System.Buffers.OperationStatus.Done || charsConsumed != hex.Length || bytesWritten != Size)
                throw new FormatException(GetString("Invalid hex format for TraceId."));
        ...
        public void ToHexFormat(Span<char> destination) {
            if (destination.Length < Size * 2) {
                throw new ArgumentException(GetString($"TraceId hex string must be {Size * 2} characters."), nameof(destination));
            }
