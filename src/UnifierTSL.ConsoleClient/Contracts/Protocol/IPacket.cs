...
namespace UnifierTSL.Contracts.Protocol {
    public interface IPacket {
        ...
        public unsafe static void Write<TSelf>(Stream stream, TSelf packet) where TSelf : unmanaged, IPacket<TSelf> {
            int bufferSize = sizeof(TSelf) + PacketHeaderSize;
            byte* bufferPtr = stackalloc byte[bufferSize];
            Span<byte> buffer = new Span<byte>(bufferPtr, bufferSize);

            *(bufferPtr + PacketLenSize) = (byte)TSelf.ID;
            int len = TSelf.WriteContent(buffer[PacketHeaderSize..], packet);
        ...
        public unsafe static void WriteManaged<TSelf>(Stream stream, TSelf packet) where TSelf : struct, IPacket<TSelf> {
            ...
            try {
                fixed (byte* bufferPtr = buffer) {
                    ...
                    *(bufferPtr + PacketLenSize) = (byte)TSelf.ID;
                    int len = TSelf.WriteContent(new Span<byte>(bufferPtr + PacketHeaderSize, bufferSize - PacketHeaderSize), packet);
                    len += PacketHeaderSize;
                    *((int*)bufferPtr) = len;
        ...
        public static int GetBytesBufferSize(ReadOnlySpan<byte> bytes) {
        ...
        public static int WriteBytes(Span<byte> buffer, ReadOnlySpan<byte> bytes) {
            int index = 0;
        ...
        public static int ReadString(Span<byte> content, out string text) {
            ...
            Span<byte> stringBytes = content.Slice(index, length);
            text = Encoding.UTF8.GetString(stringBytes);
            return index + length;
        }

        public static int ReadBytes(Span<byte> content, out byte[] bytes) {
            int length = 0;
            int shift = 0;
            int index = 0;
            byte b;
            ...
            bytes = content.Slice(index, length).ToArray();
            return index + length;
        }
    }
