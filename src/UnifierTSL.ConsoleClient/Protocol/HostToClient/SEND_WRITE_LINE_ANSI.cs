...
namespace UnifierTSL.ConsoleClient.Protocol.HostToClient
{
    ...
    public struct SEND_WRITE_LINE_ANSI(string text) : IPacket<SEND_WRITE_LINE_ANSI>
    {
        ...
        public static SEND_WRITE_LINE_ANSI Read(Span<byte> content)
        {
            IPacket.ReadString(content, out string str);
            return new(str);
        }

        public static int WriteContent(Span<byte> buffer, SEND_WRITE_LINE_ANSI packet)
        {
        ...
        {
            return nameof(SEND_WRITE_LINE_ANSI) + ':' + Text;
        }
    }
}
